...
Realtime ChatGPT voice for Apple Watch. Tap the orb, talk, hear the answer streamed back through the watch speaker. Two voice engines: OpenAI's `gpt-realtime` for true streaming speech-to-speech, and a turn-based `gpt-5.5` pipeline for cases where you want the smarter (slower) reasoning model.
...
- **WatchGPTPhone** (iOS) — holds the OpenAI WebSocket (or runs the GPT-5.5 turn pipeline), persists transcripts, and relays audio over Bluetooth.
...
- **Realtime engine** — OpenAI `gpt-realtime`, semantic VAD with barge-in. Fast, conversational.
- **GPT-5.5 engine** — phone-side speech detection → transcription → Responses API (`gpt-5.5`, reasoning effort `low`) → TTS → playback. Slower, but you get the smarter model.
...
- An OpenAI API key with access to `gpt-realtime`. If you want the GPT-5.5 engine, your account also needs access to `gpt-5.5`.
...
- **Engine** — Realtime (default, recommended) or GPT-5.5.
...
- **Realtime voice** — pick from 10 voices. `marin` and `cedar` sound best with `gpt-realtime`.
...
- **GPT-5.5 mode is noticeably slower than Realtime.** It is not realtime — it's a sequential pipeline (listen → transcribe → reason → synthesize → speak). Expect multi-second latency per turn. It exists for cases where the smarter model matters more than feel.
...
- Which engine (Realtime or GPT-5.5) and which mode (hands-free or push-to-talk).
