...
- On `session.created`, sends a `session.update` configuring 24 kHz PCM16 in/out, the iPhone-selected realtime voice, `gpt-4o-mini-transcribe` for input transcription, and either semantic VAD with the iPhone-selected eagerness plus `create_response`/`interrupt_response`, or manual turn detection for push-to-talk fallback.
...
1. UserDefaults `openAIAPIKey` (from Settings UI).
